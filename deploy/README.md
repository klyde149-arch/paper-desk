...
| Алерт «тик убит systemd» | `deploy/tick_alert.sh` через шаблон `tick-alert@.service`, подключён как `OnFailure=` к `live-tick.service`, `live-rf-tick.service` и `rf-bake.service`. Закрывает единственную дыру, которую сторожа внутри тика увидеть не могут: при `TimeoutStartSec` systemd убивает процесс, и ни `engine_watch`, ни `git_sync_watch` (он в `tick_finish`, то есть ЗА точкой смерти) не успевают выполниться — отказ полностью бесшумен (инцидент 2026-08-18, 63 мин без публикации). Текст алерта намеренно не утверждает, что позиции брошены: чаще всего движок отработал, потеряна только публикация. Проверить руками: `sudo systemctl start tick-alert@live-rf-tick.service`. |
