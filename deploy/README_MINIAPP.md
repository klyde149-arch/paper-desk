...
только читает. Развёрнут 2026-08-17.
...
Сейчас работает **quick-туннель** (TryCloudflare): аккаунт не нужен, но **URL случайный и
меняется при каждом перезапуске `cloudflared`**. При смене URL кнопки молча ведут в никуда —
это ровно тот класс тихой поломки, на котором проект уже горел. Обновить всё разом:

```bash
URL=$(journalctl -u mini-app-tunnel --no-pager -o cat | grep -oE 'https://[a-z0-9-]+\.trycloudflare\.com' | tail -1)
A=/home/trader/paper-desk/telegram-mini-app
sudo -u trader sed -i "s|^WEB_APP_URL=.*|WEB_APP_URL=$URL|" $A/.env
sed -i "s|^MINI_APP_URL=.*|MINI_APP_URL=$URL|" /etc/trading-assistant.env
cd $A && sudo -u trader npm run set-menu-button
...
Второй получатель фьючерсных отчётов (`TG_CHAT_ID_FUT`) в кабинет НЕ допущен: чтобы дать ему
только фьючерсы, его id добавляется в `owners` портфеля `rf` в `portfolios.json` (тогда сводки
он не увидит и о существовании чужих портфелей не узнает).
