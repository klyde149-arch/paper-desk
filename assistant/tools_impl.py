...
_TOOL_SCHEMAS = [
    ...
    _t('propose_close_position',
       ...
       {'asset': {'type': 'string',
                  'enum': ['BR', 'NG', 'GOLD', 'SILV', 'Si', 'CNY', 'MIX',
                           'Eu', 'COCOA', 'VTBR', 'PLD', 'SBRF', 'RTS'],
                  'description': 'актив: нефть=BR, газ=NG, золото=GOLD, серебро=SILV, '
                                 'доллар=Si, юань=CNY, мосбиржа=MIX, евро=Eu, какао=COCOA, '
                                 'втб=VTBR, палладий=PLD, сбер=SBRF. RTS выведен из торговли, '
                                 'но остаётся для закрытия старых позиций'},
