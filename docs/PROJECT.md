...
**P&L позиции.** Главная цифра — `upnl_rub`, P&L **этой позиции**: текущая цена против цены
входа по открытым сейчас лотам. Процент — **от задействованного ГО** (`lots × go_per_lot`), а
не от движения цены и не от номинала контракта.

`expected_yield` брокера в строку позиции **не годится**: он считается по КОНТРАКТУ с момента,
когда позиция по нему была нулевой. Если бот переоткрыл контракт в тот же день, не выходя в
ноль (CRU6 и EuU6 27.08), туда попадает прибыль уже закрытых сделок — а она уже посчитана в
«P&L сделок» и в таблице закрытых сделок. Замер 2026-09-02: CRU6 `expected_yield` 55 910 ₽
против 18 177 ₽ реальной переоценки позиции (`lots × (cur_px − avg_px) × ₽/пункт` по ценам
САМОГО брокера, сходится с нашим `upnl_rub` 18 095 ₽); на ГО это 23,89% против честных 7,73%.
Для EuU6 — 33 852 ₽ против 1 036 ₽. Контрактное число остаётся на дашборде отдельной
подписанной строкой «по контракту с открытия» — это то, что показывает приложение брокера.
