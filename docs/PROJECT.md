...
**Результат бота** = сведённая на клирингах вариационка + текущая несведённая + несведённая
маржа закрытых сегодня позиций − фактические комиссии. Считает `Invoke-BrokerLedger` по
`GetOperations` (раз в сутки, состояние `broker_ledger`). Сверено 2026-09-01:
...
**Механика FORTS: `var_margin` vs `var_margin_settled`, два инцидента 2026-09-09.**
`var_margin` позиции у брокера — это НЕсведённый остаток P&L по контракту с последнего
клиринга; на ночном клиринге (`$CLEARING`, ЕТС: `23:48–23:59` + `00:00–00:32` MSK) он не
обнуляется, а **переякоривается**. `var_margin_settled` — сумма, которая будет **выплачена**
на ближайшем клиринге; она замерзает на тике ~19:15 MSK и живёт отдельно от `var_margin` до
00:15 — то есть **вечерняя сессия (19:05–23:50) расчётно относится к СЛЕДУЮЩЕМУ торговому
дню**. Из этого — два независимых бага, вскрытых на боевом счёте 09.09.2026:
1. `Invoke-BrokerLedger` снимался в `23:00`, т.е. **до** клиринга, а не после — 23 часа в сутки
   «Результат бота» был завышен/занижен ровно на вчерашнюю сведённую маржу (боевой скачок в
   ночь 08→09.09: +76 692,81 ₽ ровно в 00:00 MSK). Исправлено: окно сдвинуто на `00:35–23:47`
   (`BROKER_LEDGER_FROM/TILL` в `live_rf_engine.ps1`), с ретраем до `GIVEUP=01:30`, пока
   брокер не опубликует клиринг (`last_varmargin_op_ms`, `Get-MskBoundaryMs`).
2. Маржа позиции, закрытой **внутри дня**, мгновенно пропадает из `capital_breakdown.futures`
   (позиции больше нет в портфеле) и попадёт в `varmargin_rub` только на ближайшем клиринге —
   в промежутке она не лежит НИГДЕ (боевой пример: BRV6 закрыт 09.09 в 13:22 MSK с P&L сделки
   14 172,47 ₽, при этом «висело» ≈37 тыс. ₽ брокерской вариационки). Исправлено: липкий
   снимок `broker_vm_last` (по `instrument_uid`, пишет `Set-BotCapital` каждый тик) +
   `Add-PendingSettle` при закрытии/ролле/TP1 кладёт разницу в `pending_settle`, откуда её
   забирает `Invoke-PendingSettlePrune`, как только `Invoke-BrokerLedger` увидит клиринг,
   сводящий именно её (`Get-SettledCutMs` — рубеж 19:00 MSK по дате самой операции, не
   календарный день, поэтому переживает выходные).

Тот же класс двойного счёта — в `Invoke-Governors`: `go.bot_capital_rub` (легаси, на которой
считаются hard-DD −35% и дневной −8%) в клиринговом зазоре складывает УЖЕ списанные рубли и
ЕЩЁ не переякоренный `var_margin` — фантомный провал (08.09 00:00 MSK: 1 554 364 ₽ против
реальных 1 614 486 ₽). С 09.09 `Invoke-Governors` целиком пропускается внутри `Test-Clearing`.
...
| `tools/live_rf_engine.ps1` | **Боевой C3b на Т-Инвестициях** (FORTS + TQBR). Сигналы байт-в-байт как на бумаге; отличается только исполнением: целые лоты, рубли, брокерские стоп-заявки, виртуальные леджеры рукавов поверх одного счёта. Самый большой файл репозитория (~3030 строк). |
...
| `tools/test_live_rf_scenarios.ps1` | через файл выше, самостоятельно не запускается | **77 сценариев** против движка на mock-транспорте, без сети и токена. Каждый: чистый корень, фикстуры, очередь mock, тики движка в дочернем процессе, проверки JSON. Группы: вход/отказ/потеря-adopt/repost/qty0, матрица кэпа и трима ГО, hard-DD, дневной халт, дрифты D2/D4/D5/D6, дефицит и профицит акций, клиринг, выходные, kill-файлы, флуд, TP1-синк, роллы, momentum-ребаланс, восстановление после краха, фандинг, dryrun e2e, цена входа, ребейз рукавов, вечернее подтверждение, авто-ребейз, маржиналка, long-only, окно/ретрай брокерского леджера, несведённая маржа закрытых позиций (`pending_settle`), пауза governors в клиринге |
