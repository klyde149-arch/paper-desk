...
# 2026-08: расширение с 8 до 12 по решению пользователя. Выведен RTS, добавлены
# Eu (евро/рубль), COCOA (какао), VTBR (ВТБ), SBRF (Сбербанк).
# PLD (палладий) ВЫВЕДЕН 13.09.2026: за 6,5 лет 30 сделок, 73% убыточных, PF 0,47, минус в
# 5 годах из 7; на живом контуре 3 сделки из 3 в минусе (-33 290 ₽). Разбор: ИТОГИ сессии
# 12-13.09 и docs/backtests/rf_live_27_trades_2026-09.md.
# ОГОВОРКА: на резерве и полном периоде этот набор показал себя ХУЖЕ прежней восьмёрки
# (резерв 2.11%/мес против 5.14%, полный период 4.91% против 5.90%, просадка выше).
# Решение принято сознательно, разбор в docs\backtests\slots_universe_search_2026-08.md.
$ASSETS = @('BR','NG','GOLD','SILV','Si','CNY','MIX','Eu','COCOA','VTBR','SBRF')
# 2026-08: акция VTBR выведена из momentum (было 12 бумаг, стало 11). Причина не стратегическая:
# код фьючерса ВТБ совпадает с тикером акции, а серии цен лежат в одном файле по имени -
# фьючерс в пунктах и акция в рублях затирали бы друг друга. Momentum берёт top-4, поэтому
# сокращение пула с 12 до 11 меняет только выбор кандидатов, не механику рукава.
