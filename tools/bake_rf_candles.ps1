...
function Build-RfPresentationSnapshot($State) {
  ...
  $closed = @($trades | ForEach-Object { [pscustomobject]@{ id=$_.id; asset=$_.asset; secid=$_.secid; title=(RuName $names 'fut' ([string]$_.asset) ([string]$_.secid)); side=$_.side; entryDay=$_.entryDay; entry=$_.entry; exitDay=$_.exitDay; exitPx=$_.exitPx; exitReason=$_.exitReason; pnl=$_.pnlRub; rMultiple=$_.rMultiple; fees=$_.feesRub } })
